...
class WP_Stack_CDN_Plugin extends WP_Stack_Plugin {
	...
	public function init() {
		...
		$this->extensions = apply_filters( 'wp_stack_cdn_extensions', array( 'jpe?g', 'gif', 'png', 'css', 'bmp', 'js', 'ico', 'mp3' ) );
		if ( !is_admin() ) {
	...
	public function filter_uploads_only( $content ) {
		...
		return preg_replace( "#=([\"'])(https?://{$domain})?$preg_path/((?:(?!\\1).)+)\.(" . implode( '|', $this->extensions ) . ")(\?((?:(?!\\1).)+))?\\1#", '=$1' . $this->protocol . $this->cdn_domain . $path . '/$3.$4$5$1', $content );
	}

	public function filter( $content ) {
		return preg_replace( "#=([\"'])((https?)://{$this->site_domain})?/([^/](?:(?!\\1).)+)\.(" . implode( '|', $this->extensions ) . ")(\?((?:(?!\\1).)+))?\\1#", '=$1' . $this->protocol . $this->cdn_domain . '/$4.$5$6$1', $content );
	}
