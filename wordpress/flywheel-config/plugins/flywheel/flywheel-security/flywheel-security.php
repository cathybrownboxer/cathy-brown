...
function flywheel_spam_log_process_status( $comment_id, $comment_status )
{
	...
	if (($log_file = @fopen('/www/wp-content/security.log', 'a')) === FALSE)
		return;
	...
	/* According to http://php.net/fwrite, writes less than the filesystem's
	   block size are atomic in append mode. No need for flock. Also, nothing
	   to do in case of error so not checking. */
